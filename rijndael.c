...
void gkey(int nb,int nk,u8 *key) { /* blocksize=32*nb bits. Key=32*nk bits */
...
void aes_decrypt(u8 *iv, u8 *inbuf, u8 *outbuf, unsigned long long len) {
  ...
  for (blockno = 0; blockno <= (len / sizeof(block)); blockno++) {
	...
	ctext_ptr[i] ^ block[i];
    //gprintf("Block %d output: ", blockno);
