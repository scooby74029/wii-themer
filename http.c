...
char *http_host;// = "nus.cdn.shop.wii.com";
u16 http_port ;//= 80;
char *http_path;// = "http://nus.cdn.shop.wii.com/ccs/download/0000000100000002/00000087.app";
u32 http_max_size;
...
bool tcp_read (const s32 s, u8 **buffer, const u32 length) {
	...
	while (left) {
		...
		if (block > 2048)
			block = 2048;
		//gprintf("going net read ...");
		res = net_read (s, p, block);
		//gprintf("res(%d) net read \n",res);
...
bool http_request (const char *url, const u32 max_size) {
	...
	gprintf("tcp_write returned %d\n", b);

	free (request);
	linecount = 0;

	for (linecount=0; linecount < 32; linecount++) {
		char *line = tcp_readln (s, 0xff, gettime(), 300000);
		gprintf("tcp_readln returned %p (%s)\n", line, line?line:"(null)");
		if (!line) {
			http_status = 404;
	...
	//	gprintf("here is prob \n");
	//	gprintf("clen(%d) httpmsize(%d) \n",content_length,http_max_size);
		result = HTTPR_ERR_TOOBIG;
		net_close (s);
		return false;
	}
	//gprintf("going to read tcp \n");
...
bool http_get_result (u32 *_http_status, u8 **content, u32 *length) {
	...
	//gprintf("content(%s) length(%d) http path(%s) \n",*content,*length,http_path);
	free (http_host);
	free (http_path);
